...
export default function ThemeSwitcher() {
  const changeTheme = useDashboardThemeState((state) => state.changeTheme);
  const mode = useDashboardThemeState((state) => state.mode);

  return (
    <FormControlLabel
      checked={mode === THEME_MODE.DARK}
      control={<MaterialUISwitch onChange={(_, checked) => changeTheme(checked ? THEME_MODE.DARK : THEME_MODE.LIGHT)} sx={{ m: 1 }} defaultChecked />}
      label="MUI switch"
    />
  )
}